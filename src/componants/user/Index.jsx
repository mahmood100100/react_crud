...
const getUsers2 = async()=>{
  let {data} = await axios.get('https://crud-users-gold.vercel.app/users')
  setUsers(data.users)
  setLoader(false)
}
  ...
  <tbody>
   {users.length > 0 ? users.map((user , index)=>
      ...
      <tr>
        ...
        <td>{user.email}</td>
        <td>{user.password}</td>
        <td><button className=' bg-danger' onClick={()=>deleteUser(user._id)}>Delete</button></td>
        <td><button className=' bg-info'><Link className=' text-decoration-none text-dark' to={`/user/${user._id}`} >Details</Link></button></td>
        <td><button className=' bg-warning'><Link className=' text-decoration-none text-dark' to={`/user/edit/${user._id}`} >Edit</Link></button></td>
      </tr>
      </React.Fragment>
   ) :<h2>No users data</h2>}
  </tbody>
